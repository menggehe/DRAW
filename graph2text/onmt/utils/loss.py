...
def build_loss_compute(model, Discriminator,model2, tgt_field, opt, train=True):
    """
    ...
    if opt.copy_attn:
        ...
        compute = NMTLossCompute(
            criterion, loss_gen, Discriminator,model2, lambda_coverage=opt.lambda_coverage,
...
class LossComputeBase(nn.Module):
    ...
    def __init__(self, criterion, generator, Discriminator,model2):
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, criterion, generator, Discrminator, model2,normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0):
        super(NMTLossCompute, self).__init__(criterion, generator, Discrminator,model2)
    ...
    def _compute_loss(self, batch, output, target, std_attn=None,
        ...
        out_ = scores.max(2)[1]
        pred = self.model2(out_, target, batch_model2)
        pred_list = pred.cpu().detach().numpy().tolist()
        list = []
        for i in range(len(pred_list)):
            list.append(pred_list[i][i])
        seq_len = len(out_[0])
        # pred = pred.item()
        # pred = pred.cpu().data[:, 1].numpy()
        rewards = []
        ...
        rewards = torch.tensor(rewards).cuda()
        rewards = rewards.transpose(0, 1)
        # rewards = rewards.repeat(out_.size(0),1)

        gtruth = target.transpose(0, 1)

        loss2 = self.criterion(scores.contiguous().view(-1, scores.size(2)), gtruth.contiguous().view(-1), rewards)

        loss = loss + 0.5*loss_1 + loss2
